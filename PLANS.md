...
## 목표
- 주문 이후 상품명이나 가격이 변경되어도 주문 당시 상품명, 단가와 총액이 유지된다.
- 주문 상태는 정의된 방향으로만 전이되고 취소된 주문의 재고는 한 번만 복구된다.
- 주문에 사용하는 재고 증감 수량은 양수이며 재고는 음수가 되지 않는다.

## 현재 상태
- 현재 브랜치: BE `codex/order-stock-consistency`
- 관련 저장소: BE `/Users/shinjeongbeom/SpringProject/E-Commerce`
- 기존 구현: `OrderItem.orderPrice`에 단가를 저장하지만 상품명 스냅샷과 상태 전이 규칙은 없다.
- 기존 동시성: 상품 조회에 비관적 쓰기 락을 사용한다.

## 범위
- 포함: 주문 상품명·단가 스냅샷, 주문 총액 계산, 주문번호 유일성, 상태 전이 규칙, 취소 주문 락, 재고 증감 불변식, 회귀 테스트
- 제외: 락 전략 성능 비교, Toss 결제 재설계, 결제 취소·환불, 운영 DB migration, API 응답 확장

## 작업 순서
1. 주문·재고 Entity와 Service의 현재 규칙 및 사용처 조사
2. 주문 스냅샷과 상태 전이 규칙을 도메인 코드로 이동
3. 주문 취소와 재고 복구의 트랜잭션 경계 보강
4. 가격 변경, 잘못된 상태 전이, 재고 불변식 회귀 테스트 작성
5. 관련 테스트와 전체 빌드 검증

## 검증
- `./gradlew test --tests 'com.jeongbeom.ecommerce.order.*'`
- `./gradlew test`
- `./gradlew build`
- 상품 변경 후에도 저장된 주문 스냅샷과 총액이 유지되는지 확인
- 중복 취소 시 재고가 한 번만 복구되는지 확인

## 커밋 분리
- `feat : 주문 상품 스냅샷과 상태 전이 규칙 적용`
- `test : 주문·재고 정합성 회귀 테스트 추가`

## 리스크
- 남은 위험: 현재 비관적 락의 처리량과 대기시간은 Task 2에서 실제 MySQL 부하 테스트로 비교한다.
- 롤백 방법: 본 브랜치 커밋을 revert하고 기존 Entity와 Service 규칙으로 복원한다.
